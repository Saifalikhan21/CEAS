...
class Scenario:
    name: str
    duration_s: int = 600
    emg_rate: float = 0.02  # fraction of emergency/public-service vehicles

class AIMToySim:
    """
    A toy tick-based intersection simulator (AIM-like). Not physics-accurate.
    Produces per-vehicle waits and emergency on-time stats for MVP charts/tables.
    """
    ...
    def _rates(self):
        # arterial vs balanced demand profiles (vehicles/s per approach)
    ...
    def run(self, controller_fn, controller_name: str, T_on_time: float = 20.0):
        ...
        emg_log = []   # list of dicts: {t_arr, approach, served?, t_serv}
        throughput = 0
        ...
        for t in range(T):
            ...
            cap = 5  # max vehicles served per phase decision this tick (toy cap)
            ...
            # accumulate simple waits and throughput
            for a, s in served.items():
                throughput += s
                if s > 0:
                    # sample approximate per-vehicle waits (toy)
                    approach_waits[a].extend(self.rng.uniform(1, 30, size=s))

            # mark emergency served if their approach was served this tick
            for emg in emg_log:
